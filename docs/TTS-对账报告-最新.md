...
> 生成时间：**2026-05-30 02:29 中国标准时间**  
> 命令：`python scripts/report-tts-audit.py`

## 结论（发布门禁）

- **通过**：需朗读 **2685** 条，根目录 MP3 **4680** 个，缺失 **0**
- **发布包**：`发布包/tts-cache/` 缺 **1** 条（请再跑 `生成语音包.bat` 同步双目录）
...
|------|------|
| 需朗读（喇叭侧） | 2685 |
| 根目录 tts-cache MP3 | 4680 |
| 缺失 MP3（根目录） | 0 |
| 多余 MP3（历史遗留） | 1995 |
| registry.requiredCount | 2685 |
